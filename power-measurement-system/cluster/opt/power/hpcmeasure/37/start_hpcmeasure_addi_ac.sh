...
if [ "$PROFILE_ID" = "1" ] || [ "$PROFILE_ID" = "2" ] || [ "$PROFILE_ID" = "1_2" ] ; then
  CONFIG_INI=$HPCMEASURE_PATH/profiles/config_${NODE}_ac_${PROFILE_ID}.ini
  POWER_CONFIG_INI=$HPCMEASURE_PATH/profiles/config_components_${NODE}_ac_${PROFILE_ID}.ini
else
...
#no config file -> simple exit
if [ ! -f ${POWER_CONFIG_INI} ]; then
  echo $DATE":no valid POWER_CONFIG_INI provided: ${POWER_CONFIG_INI} ">> $LOG_FILE
  exit 0
fi
...
echo "$CONVERT_BIN -r $RAW_FILE_PATH -c $POWER_CONFIG_INI -d $DBKEY -s ${POWER_SAVE_ON}  -u ${PBS_USER} -n ${PBS_JOBNAME} -t 1 -p ${PLATFORM_NAME}  &>/dev/null" >>$LOG_FILE
$CONVERT_BIN -r $RAW_FILE_PATH -c $POWER_CONFIG_INI -d $DBKEY -s ${POWER_SAVE_ON} -u ${PBS_USER} -n ${PBS_JOBNAME} -t 1 -p ${PLATFORM_NAME} &>/dev/null & 
