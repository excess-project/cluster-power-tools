...
if [ -z "$PBS_JOBID=" ]; then
  echo $DATE":Error in stop_hpcmeasure_compnode.sh: variable PBS_JOBID is not initialized" >> $LOG_FILE
  exit 1
fi
...
#echo $DATE":stop_hpcmeasure_compnode.sh service ${HPCM_SERVICE} on addi system for ${PBS_USER} ${PBS_JOBID} ${DBKEY} ${NODE} ${PROFILE_ID} ${POWER_SAVE_ON}  is stoped" >> $LOG_FILE
