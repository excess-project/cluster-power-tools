...
HPC_COPY_ADDI_SCRIPT=$HPCMEASURE_PATH/copy_hpcmeasure_addi.sh
...
LOG_FILE=/var/log/hpcmeasure/addi_epologue.log
DATE="$( date +'%c' )"

if [ -f ${HPCMEASURE_PATH}/stop_hpcmeasure_addi_ac.sh ]; then
  echo $DATE": start stop_hpcmeasure_addi_ac.sh" >> $LOG_FILE
  ${HPCMEASURE_PATH}/stop_hpcmeasure_addi_ac.sh  ${PBS_USER}  ${PBS_JOBID} ${DBKEY} ${NODE} ${PROFILE_ID} ${SAVE_POWER_DATA}
fi
