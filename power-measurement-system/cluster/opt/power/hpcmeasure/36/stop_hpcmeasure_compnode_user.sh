...
echo $DATE":--start initializing part of stop_hpcmeasure_compnode.sh---"
...
echo $DATE":CONFIG_INI:"$CONFIG_INI 
echo $DATE":POWER_FILE_PATH:"$POWER_FILE_PATH  
echo $DATE":RAW_FILE_PATH:"$RAW_FILE_PATH  
...
ssh root@addi "/sbin/service ${HPCM_SERVICE} stop ${PBS_USER} ${PBS_JOBID} ${DBKEY} ${NODE}" dreamcloud 0
echo "ssh power@addi "/sbin/service ${HPCM_SERVICE} stop ${PBS_USER} ${PBS_JOBID} ${DBKEY} ${NODE}" dreamcloud 0"
echo $DATE":stop_hpcmeasure_compnode.sh service ${HPCM_SERVICE} on addi system for  ${DBKEY} ${NODE} is stoped" 
