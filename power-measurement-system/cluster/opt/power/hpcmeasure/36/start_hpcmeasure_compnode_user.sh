...
ssh power@addi "/sbin/service ${HPCM_SERVICE} start ${DBKEY} ${NODE} ${PROFILE_ID} ${POWER_SAVE_ON}"
echo $DATE":start_hpcmeasure_compnode.sh ${HPCM_SERVICE} is started" >> $LOG_FILE
echo $DATE":service ${HPCM_SERVICE} start ${DBKEY} ${NODE} ${PROFILE_ID} ${POWER_SAVE_ON} " >> $LOG_FILE
