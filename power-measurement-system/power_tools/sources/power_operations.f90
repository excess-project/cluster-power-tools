...
!calculate norm2
function power_norm2(vector) result(ret_norm2)
  real(kind=rdk), dimension(:), intent(in) :: vector
  real(kind=rk) :: ret_norm2
  integer(kind=ik) :: ii, length
  real(kind=rdk) :: sum_el
  
  length=size(vector,dim=1)
  sum_el=0.0_rdk
  do ii=1_ik,length
    sum_el= sum_el + vector(ii)*vector(ii)
  end do
  ret_norm2 = real(sqrt(sum_el),kind=rk)
end function power_norm2
...
subroutine power_operation_sub(power_data,norm_2,power_data1,alpha,power_data2,input_parameter,err_code)
  ...
  norm_2 = power_norm2(power_data%rawdata(1:rawdata_length))
...
subroutine power_operation_find_min_alpha(alpha, norm_2, power_data,power_data1,power_data2,input_parameter,err_code)
  ...
  norm2_data1 = power_norm2(power_data1%rawdata(1:rawdata_length))
  norm2_data2 = power_norm2(power_data2%rawdata(1:rawdata_length))
