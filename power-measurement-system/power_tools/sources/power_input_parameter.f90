...
type power_input_parameter_type
  ...
  integer(kind=ik)        :: window_step_left_a
  integer(kind=ik)        :: window_step_right_a
  ...
  integer(kind=ik)              :: num_operation_repeat
  character(len=str_length)     :: out_diff_prefixchannel
  character(len=str_length)     :: config_file
  character(len=str_length)     :: profile_topdir
  character(len=str_length)     :: profile_file
  character(len=str_length)     :: out_filename
  character(len=str_length)     :: in_filename
  character(len=str_length)     :: out_idx_filename
...
type(power_input_parameter_type),intent(in) :: input_parameters
  ...
  write(power_output_unit,*)"window_step_left:",input_parameters%window_step_left
  write(power_output_unit,*)"window_step_right_a:",input_parameters%window_step_right_a
  write(power_output_unit,*)"window_step_right_a:",input_parameters%window_step_right_a
  ...
  write(power_output_unit,*)"out_filename:",input_parameters%out_filename
  write(power_output_unit,*)"in_filename:",input_parameters%in_filename
  write(power_output_unit,*)"out_idx_filename:",input_parameters%out_idx_filename
  write(power_output_unit,*)"relative_time:",input_parameters%relative_time
  write(power_output_unit,*)"num_operation_repeat",input_parameters%num_operation_repeat
...
type(power_input_parameter_type),intent(inout) :: input_parameters
  ...
  input_parameters%channel_id = 0_ik
  input_parameters%window_step_left = 5_ik
  input_parameters%window_step_right = 5_ik
  input_parameters%window_step_left_a = 1_ik
  input_parameters%window_step_right_a = 1_ik
  ...
  input_parameters%num_operation_repeat = 1_ik
  ...
  write(input_parameters%config_file,'(A)') "undef_config_file"
  write(input_parameters%profile_topdir,'(A)') "undef_profile_topdir"
  write(input_parameters%config_file,'(A)') "undef_profile_file"
  write(input_parameters%config_file,'(A)') "undef_profile_file"
  write(input_parameters%out_filename,'(A)') "out_filename"
  write(input_parameters%out_filename,'(A)') "in_filename"
  write(input_parameters%out_idx_filename,'(A)') "out_idx_filename"
...
subroutine power_input_parameter_read(input_parameters, err_code)
  ...
  message='parameters of power consumption module' &
      ...
      &//' -channel_id <int>' &
      &//' -board_id <int>' &
      &//' -num_operation_repeat <int>' &
      &//' -window_step_left <int>' &
      &//' -window_step_right <int>' &
      &//' -window_step_left_a <int>' &
      &//' -window_step_right_a <int>' &
      &//' -average_window <int>' &
      &//' -alpha_min <real>' &
      &//' -alpha_max <real>' &
      &//' -alpha_step <real>' &
  ...
  if(power_extract_command_parameter(cmd,'-profile_topdir',stop_on_error=.false.,&
                value=input_parameters%profile_topdir,&
                syntax=message) /=0) then
  endif
  if(power_extract_command_parameter(cmd,'-profile_file',stop_on_error=.false.,&
                value=input_parameters%profile_file,&
                syntax=message) /=0) then
  endif
  if(power_extract_command_parameter(cmd,'-out_filename',stop_on_error=.false.,&
                value=input_parameters%out_filename,&
                syntax=message) /=0) then
  endif
  if(power_extract_command_parameter(cmd,'-in_filename',stop_on_error=.false.,&
                value=input_parameters%in_filename,&
                syntax=message) /=0) then
  endif
  if(power_extract_command_parameter(cmd,'-out_idx_filename',stop_on_error=.false.,&
                value=input_parameters%out_idx_filename,&
                syntax=message) /=0) then
  endif
  ...
  if(power_extract_command_parameter(cmd,'-num_operation_repeat',stop_on_error=.false.,&
                value=input_parameters%num_operation_repeat,syntax=message) /=0) then
  endif
  ...
  if(power_extract_command_parameter(cmd,'-window_step_left_a',stop_on_error=.false.,&
                value=input_parameters%window_step_left_a,syntax=message) /=0) then
  endif
  if(power_extract_command_parameter(cmd,'-window_step_right_a',stop_on_error=.false.,&
                value=input_parameters%window_step_right_a,syntax=message) /=0) then
  endif
